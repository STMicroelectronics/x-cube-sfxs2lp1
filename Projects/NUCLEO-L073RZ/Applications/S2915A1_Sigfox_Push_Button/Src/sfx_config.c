...
void RadioIRQEnable(uint8_t state, uint8_t edge_direction)
{
  ...
  if(edge_direction == 0)
  {
    S2915A1_RADIO_GPIO_Init_Update(S2LP_RADIO_GPIO, RADIO_MODE_EXTI_IN,FALLING);
  }
  else if(edge_direction == 1)
  {
    S2915A1_RADIO_GPIO_Init_Update(S2LP_RADIO_GPIO, RADIO_MODE_EXTI_IN,RISING);
  }
  else
  {
    S2915A1_RADIO_GPIO_Init_Update(S2LP_RADIO_GPIO, RADIO_MODE_EXTI_IN,FALLING);
  ...
  {
    /* uC IRQ Disable */
    S2915A1_RADIO_IoIrqDisable(GpioIrq);
  }
}
...
{
  uint8_t  status;
  status = S2915A1_EEPROM_ReadPage(EEPROM_INSTANCE,nAddress, cNbBytes, pcBuffer);
...
{
  uint8_t status;
 status = (uint8_t) S2915A1_EEPROM_WritePage(EEPROM_INSTANCE, nAddress, cNbBytes,pcBuffer);
                        ...
                        uint8_t can_return_before_tc)
{
  ...
  S2915A1_SPI_SendRecv(pInBuffer, pOutBuffer_, cNbBytes);
